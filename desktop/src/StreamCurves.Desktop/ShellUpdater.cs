...
/// <summary>
/// Shell self-updates via Velopack against the STAF repo's GitHub releases. Only shell releases
/// are normal (non-prerelease) releases (payload releases are always prereleases), so
/// prerelease:false resolves to the newest installer release. No channel is set here: Velopack
/// defaults to the channel the running package was packed with (`--channel streamcurves` in
/// streamcurves-shell.yml), which is what keeps this app's feed (releases.streamcurves.json)
/// apart from any other desktop app published from the same repository. No-ops for unpackaged
/// dev builds.
/// </summary>
