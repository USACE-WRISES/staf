...
public sealed record SupervisorOptions
{
    ...
    /// <summary>First-ever start on this machine: StreamCurves' geopandas/rasterio/HyRiver import chain is slow.</summary>
...
public sealed class AppSupervisor : IAsyncDisposable
{
    ...
    private int Spawn(Entry entry, PayloadPaths payload, int attempt)
    {
        ...
        // GetFullPath normalizes separators so the cwd logged below is the one the app sees.
        ...
        entry.Log.WriteLine($"[shell] env overrides: {string.Join(", ", spec.Environment.Where(kv => kv.Value is not null).Select(kv => kv.Key))}");
        var removed = spec.Environment.Where(kv => kv.Value is null).Select(kv => kv.Key).ToList();
        if (removed.Count > 0)
        {
            entry.Log.WriteLine($"[shell] env removed: {string.Join(", ", removed)}");
        }
