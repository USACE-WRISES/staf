...
namespace StreamCurves.Desktop.Core.Manifest;

/// <summary>One launchable app, as described by desktop-manifest.json (desktop/scripts/gen_desktop_manifest.py; StreamCurves ships one).</summary>
    ...
    public static DesktopManifest Parse(string json, string source = "desktop-manifest.json")
    {
        ...
        if (manifest.SchemaVersion != 1)
        {
            throw new ShellException(
                $"The app manifest ({source}) uses schema version {manifest.SchemaVersion}; this version of StreamCurves Desktop understands version 1. Update StreamCurves Desktop.");
        }
        ...
        {
            // StreamCurves lives in a subfolder of the apps root (stream-curves\, beside library\),
            // so a dir must name a real child folder: "." and ".." are both refused.
