...
/// <summary>
/// The payload manifest (latest-desktop.json) published on the rolling `streamcurves-current`
    ...
    public static LatestManifest Parse(string json, IReadOnlyList<string> allowedUrlPrefixes, string source = "latest-desktop.json")
    {
        ...
        if (manifest.SchemaVersion != 1)
        {
            throw new ShellException(
                $"The update manifest uses schema version {manifest.SchemaVersion}; this version of StreamCurves Desktop understands version 1. Update StreamCurves Desktop.");
        }
