...
public sealed class AppSupervisorTests
{
    ...
    public async Task Start_BecomesRunning_WhenProbeTurnsHealthy()
    {
        ...
        await world.Supervisor.StartAsync("streamcurves");
        ...
        Assert.True(world.StateStore.HasStartedOk("streamcurves"));
        ...
        Assert.Equal("1", spec.Environment["STREAMCURVES_DESKTOP"]);
    }
    ...
    public async Task Stop_IsGraceful_WhenHelperWorks()
    {
        ...
        await world.Supervisor.StopAsync("streamcurves");

        var state = world.Supervisor.GetState("streamcurves");
