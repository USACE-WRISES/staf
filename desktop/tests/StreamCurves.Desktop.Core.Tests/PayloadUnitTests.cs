...
{
    private readonly string _dir = Path.Combine(Path.GetTempPath(), "streamcurves-desktop-tests", Guid.NewGuid().ToString("N"));
...
public sealed class ExtractorTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "streamcurves-desktop-tests", Guid.NewGuid().ToString("N"));
