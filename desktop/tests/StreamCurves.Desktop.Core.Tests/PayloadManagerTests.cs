...
{
    private readonly string _root = Path.Combine(Path.GetTempPath(), "streamcurves-desktop-tests", Guid.NewGuid().ToString("N"));
    ...
    public async Task FirstRun_InstallsBothComponents_AndCommits()
    {
        ...
        var apps = MakeZip("apps-2026.07.06-abc1234", Path.Combine("stream-curves", "app.py"), "app = None");
        ...
        Assert.True(File.Exists(Path.Combine(_state.DirFor("apps-2026.07.06-abc1234"), "stream-curves", "app.py")));
    ...
    public async Task AppsOnlyUpdate_CarriesEnvForward_AndRecordsPrevious()
    {
        ...
        var env = MakeZip("env-cp312-aaaa1111", Path.Combine("python", "python.exe"), "fake python");
        var apps2 = MakeZip("apps-2026.07.07-def5678", Path.Combine("stream-curves", "app.py"), "app = 2");
    ...
    public async Task InstallFromDirectory_OfflineBundle_Works()
    {
        ...
        var apps = MakeZip("apps-2026.07.10-fff2222", Path.Combine("stream-curves", "app.py"), "a");
        ...
        Assert.True(File.Exists(Path.Combine(_state.DirFor("apps-2026.07.10-fff2222"), "stream-curves", "app.py")));
    }
    ...
    public async Task InstallFromDirectory_ResolvesGithubUrlsToLocalFilenames()
    {
        ...
        var apps = MakeZip("apps-2026.07.12-off8888", Path.Combine("stream-curves", "app.py"), "a");
        var json = JsonSerializer.Serialize(new
        {
            schemaVersion = 1,
        ...
        Assert.True(File.Exists(Path.Combine(_state.DirFor("apps-2026.07.12-off8888"), "stream-curves", "app.py")));
    }
    ...
    public async Task Prune_SkipsInUseDirs_DuringApply()
    {
        ...
        var env = MakeZip("env-cp312-aaaa1111", Path.Combine("python", "python.exe"), "fake python");
        var apps2 = MakeZip("apps-2026.07.11-ggg3333", Path.Combine("stream-curves", "app.py"), "app = 3");
