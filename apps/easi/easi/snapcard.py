...
info icon. Two short lines say what happened and which reach supplies the
borrowed values (one line, the legend's vocabulary); the COMID and the
drainage-area ratio live in the tip, two sentences, so the card never reads as
an error (2026-09-07: the tip lost its paragraph about what scores at the
point). The line predicting how long the engine would take was dropped on
2026-09-08 with the same note in SFARI and DEEP: the busy row reports the wait
while it happens. The routing payload is the ``siteAnchor`` from
...
def hr_snap_card(anchor: dict) -> dict:
    ...
    # One line at the pane's width, in the legend's words ("Downstream reach").
    line2 = ("", f"Downstream reach: {reach_name}, {routed + ' ' if routed else ''}downstream.")
    ...
    tip_html = ('<div class="easi-tip-title">Downstream reach</div>'
                f'<div class="easi-tip-sec">{escape(why)}</div>')
    return {"lines": [line1, line2], "tip_html": tip_html}
