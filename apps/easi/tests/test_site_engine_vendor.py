"""Drift gate for the vendored site engine (clone of the SFARI/DEEP pattern):
...
_VENDOR = Path(__file__).resolve().parents[1] / "easi" / "_vendor" / "site_engine"
...
def test_vendored_vocabulary_imports_without_the_geo_stack():
    # The vocabulary module is what the labels use; importing it must not
    # pull the delineation stack (the package resolves compute_site lazily).
    from easi._vendor.site_engine import ENGINE_ID, naming
    assert ENGINE_ID == "site-engine"
