...
subtitle: "Historical method documentation, verification, and validation"
abstract: >
...
::: callout-note-soft
**Historical validation scope, recorded 2026-09-15.** This report preserves
the methods, cached cases, figures and field comparisons preceding the
regional-criteria decision of 2026-09-14. Its midpoint mapping produces
function scores of 13 / 8 / 3. Current EASI retains the same
0.85 / 0.545 / 0.195 anchors and 13 / 8 / 3 scores, with regional reference
criteria by default. The validation results below have not been rerun under
the regional criteria and do not establish validation of those changes. The current
[regional reference criteria](https://usace-wrises.github.io/staf/walkthroughs/easi/#regional-reference-criteria)
are documented in the walkthrough. Selecting legacy criteria preserves the
former rating rules with the same anchors.
:::
...
resolution is handled by a fixed rule: the STAF site engine computes its HR reach
watershed, and the three reach-keyed metrics come from the nearest covered reach
downstream, each labeled with that reach and the drainage-area ratio between the
two streams.
...
draws cyan, the STAF site engine delineates the HR reach watershed, the
drainage area of the high-resolution reach the click snaps to, and computes the same quantities
...
so they come from the nearest covered reach downstream, and each of them is
labeled with that reach, the routed distance, and the drainage-area ratio,
whatever that ratio is. Every rating in the report and
...
probability at 0.50 and 0.25. These are EASI integration tiers. The source
...
perpendicular to the reach at one-tenth intervals from one-tenth to nine-tenths
of its length, samples each at the elevation model's native spacing, and scores
each ratio on the reach median of the nine sections. The section nearest both
medians is drawn, and the application can step through the others. Bankfull
width and depth come from the regional curves noted above.
...
at least 0.3 m on the 10 m model, 0.1 m on 1 m lidar, and at least 25 percent
of the bankfull depth. The bank is then
...
wall) contributes nothing. When the lower bank sits above twice the bankfull
depth, or no side breaks, the low bank is capped at the floodprone stage and the
ratio is reported as at least 2.0. These constants are screening conventions. A computed
...
For the historical runs documented here, each rating maps to a function
score on a 0 to 15 scale. A rating maps to the
...
::: {.callout-note}
## Statistics current as of 2026-09-16

The statistics in this section were computed with the frozen criteria of
September 2026 (method digest b2e3033116e3), from a fresh run of all 17 sites
with service data pulled on 2026-09-16. Two cautions from that work: the dam
inventory was read from a copy pulled on 2026-09-10 because the live service
was down that day, and paired same-day requests in August returned different
catchment-scale StreamCat components at one site. Small differences in the
...
- **Most evidence is landscape data.** At the 17 verification sites, StreamCat
  variables supplied 10 of 20 ratings on average, and removing them would move
  1 of 17 sites across a condition class, although the index moves more than
  for any other source. EASI results carry the strengths and
  the blind spots of national landscape summaries.
- **No reach-scale observation enters the automated result.** The earlier
  configuration consulted connected NRSA observations, which supplied about three
  ratings across all 17 verification sites, and the September 2026 revision retired
  that tier. In practice ratings come from StreamCat, the elevation model, the
  modeled monthly flows, WQP, ATTAINS, NAS, and NID.
- **One reach of cross sections drives four physical functions.** The bank-height
  and entrenchment ratios are reach medians of nine DEM transects at the
  elevation model's native resolution (1 m where published, otherwise 10 m).
  Dropping those four ratings shifts the mean index by
  about 0.01 and changes 3 of 17 site classes, so verifying or editing the
...
- **Three-bin ratings compress the index.** Every metric scores 0.85, 0.545,
  ...
  sites the EASI index spans about 0.39 to 0.73 while the paired field values
...
- **Streams outside the StreamCat network are assessed differently.** Their
  watershed metrics come from the STAF site engine's HR reach watershed rather
