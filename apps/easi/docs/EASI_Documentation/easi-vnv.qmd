...
(`python scripts/build_method_tables.py`). Breakpoints described as provisional are STAF
screening transitions, not evidence of an ecological threshold; those described as
published carry the citation given in the source register.
...
::: {.callout-important}
## These results predate the current method set

The statistics in this section were computed with the metric methods EASI used before
the 2026-07-18 revision. That revision replaced the flow-alteration calculation, the
bank-height recurrence transform, the substrate and biological-integrity surrogates, and
the thermal and nutrient methods, and it added connected NRSA evidence. The agreement
reported below therefore describes the previous methods, not the ones documented in
@sec-quantification. Refreshing it requires re-running the SFARI site corpus
(`python scripts/build_doc_assets.py`, which needs network access to the federal
services) and is outstanding.
:::
