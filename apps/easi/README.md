...
The saved baseline and both current criteria sets use 13/8/3 scores for
Good / Fair / Poor. The `legacy` switch retains the previous criteria.
Baseline comparisons show criteria and input-evidence changes, with coverage
changes reported separately.
...
These rating anchors are the midpoints of the Good (0.70 to 1.00), Fair
(0.40 to 0.69) and Poor (0.00 to 0.39) bands. Multiplying by 15 and rounding
gives 13, 8 and 3. Both regional and legacy criteria use this mapping. The
twenty functions, CWA weights, rollup formulas and 0.39 / 0.69 outcome
boundaries retain their existing definitions.
...
the same 0.85 / 0.545 / 0.195 anchors. Invalid set names raise an error.
...
validation record** with cached cases and the same 13 / 8 / 3 mapping. Its
scope notice is dated 2026-09-15. Those results have not been revalidated under
the regional criteria. The canonical source is
