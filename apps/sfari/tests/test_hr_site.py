...
def test_every_click_snaps_to_the_hr_network(monkeypatch):
    ...
    assert res["hit"][3] == 750012345 and res["lat"] == 40.31 and res["hrStatus"] == "ok"
    status, fc = hr_site.hr_flowlines_status(-83.1, 40.3, -83.0, 40.4)
    assert status == "ok" and fc["features"][0]["properties"] == {"nhdplusid": 1}
