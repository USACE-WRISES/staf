...
def test_engine_provenance_fields_default_and_roundtrip():
    ...
    r = EvidenceResult("m", origin="streamcat", anchor_label="nearest StreamCat reach, COMID 1",
                       fallback_reason="STAF site engine refused: over budget",
                       upgrade_pending=False, status="ok")
    d = r.to_dict()
    assert d["origin"] == "streamcat" and d["anchor_label"].startswith("nearest StreamCat reach")
