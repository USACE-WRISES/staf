...
Phase 3 covers the metrics backed by existing EASI clients (StreamCat / NID / WQP /
NHDPlus VAA). The remaining Table-22 metrics arrive in Phase 4 (new clients) and
Phase 5 (cross-section hydraulics).
"""
...
def ev_transport_capacity(ctx):
    ...
    if slope is None and ag is None:
...
def ev_land_use_change(ctx):
    ...
    if a is None or b is None:
        return EvidenceResult(mid, status="unavailable", source="EPA StreamCat (NLCD 2001/2019)",
...
async def pull(ctx_inputs: dict, *, progress: Optional[dict] = None) -> dict:
