...
def person(state=None) -> str:
    """Who is recorded as making a decision or an edit: the open project's initials, ``n/a``
    when none are set, never the login (``views.state.recorded_by``)."""
...
def publish_block_reason() -> str | None:
    ...
    return lib.publish_gate_reason() if lib.is_canonical_root() else None
...
def easi_page_server(input, output, session, state: AppState):
    ...
    def _pkg_detach():
        ...
        if ref is None:
            return
        _apply(ev.detach(p, ref["packageId"], by=person(state)),
    ...
    async def _run_install(path: Path | None, *, ref: dict | None = None):
        ...
        try:
            ...
            if old is not None and not evs.matches(got, old):
                ...
                return
            new = ev.attach(p, new_ref, by=person(state))
            if new is not p:
                with reactive.isolate():
    ...
    def _alt_select_open():
        ...
        _modal(f"Select for {r['functionName']}",
               ...
               ui.input_text(ns("who"), "Your initials", value=person(state), width="100%"),
    ...
    def _alt_import():
        p = _get()
        try:
            new = alts.import_alternatives(p, alts.STUDY_DIR, imported_by=person(state), at=_now())
    ...
    def _publish_form(p, pending):
        ...
        form = ui.div(
            ...
            ui.div(f"Recorded as published by {person(state)} into {lib.library_root()}.",
    ...
    def _fork():
        ...
        if p is None or p.is_revision():
            return
        new = eio.fork(p, by=person(state), version=next_version(p))
    ...
    def _curve_apply():
        ...
        try:
            ...
            new = edit.set_curve_points(p, name, stratum, pts, by=person(state),
                                        reason=reason)
    ...
    def _edge_apply():
        ...
        try:
            ...
            new = edit.set_band_edge(p, r["methodKey"], r["input"], int(_target["e"]), float(value),
                                     owner=input.edge_owner(), by=person(state), reason=reason)
    ...
    def _regional_apply():
        ...
        try:
            ...
            new = edit.set_regional_edges(p, r["methodKey"], r["input"], r["region"], float(good),
                                          float(poor), by=person(state), reason=reason)
        except edit.EditError as exc:
            _modal_err.set(str(exc))
            return
        ui.modal_remove()
    ...
    def _publish():
        ...
        try:
            version = eio.publish(p, author=person(state), revision_notes=notes,
    ...
    def _renumber():
        ...
        new.history.append({"action": "renumber", "at": _now(), "by": person(state),
