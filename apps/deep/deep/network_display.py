...
A port of EASI's ``easi/network_display.py`` (2026-09-07). The NHDPlus HR
geometry is drawn once and split by the click rule: a stretch within the snap
tolerance of an NHDPlus V2 reach is where the StreamCat lookup engine answers
by that COMID and draws dark blue; the rest is answered only by the STAF site
engine and draws cyan (its COMID-keyed values come from the nearest covered
...
``flowlines.nearest_point_on_lines``). Pure functions, no Shiny;
``fetch_streams`` takes injectable fetchers so the split is tested offline.
"""
...
def _default_display(west: float, south: float, east: float, north: float,
    ...
    v2_fc, hr_fc = _fetch_pair(bbox, flowlines.flowlines_in_bbox, hr_site.hr_flowlines_fc)
...
def fetch_streams(bbox: tuple, *, tol_ft: float = 150.0, fetch_v2: Optional[Fetcher] = None,
    ...
    v2_fc, hr_fc = _fetch_pair(bbox, fetch_v2 or flowlines.flowlines_in_bbox,
                               fetch_hr or hr_site.hr_flowlines_fc)
    return _assemble(bbox, v2_fc, hr_fc, tol_ft)
...
def v2_reach_feature(comid: int) -> Optional[dict]:
    ...
    fabric API, for the StreamCat-reach highlight when the reach lies outside
    the viewport's V2 fetch (a routed click). None when unknown or unanswered."""
    try:
