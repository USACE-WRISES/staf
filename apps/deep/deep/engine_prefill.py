...
_VENDOR_ROOT = "deep._vendor.site_engine"
...
def engine_version() -> Optional[str]:
    if not site_engine_available():
        return None
    from deep._vendor import site_engine
...
def run_engine(lat: float, lon: float, *, families: Optional[list[str]] = None,
    ...
    try:
        from deep._vendor.site_engine.engine import compute_site
        from deep._vendor.site_engine.provenance import INTERACTIVE_CONFIG
