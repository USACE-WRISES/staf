...
def test_the_published_regions_announce_what_they_score():
    """The three methodology 0.12 versions: 29, 29 and 21 metrics, not 36, 36 and 28."""
    for aid, expected in (("northeastern-highlands", "29 metrics"),
                          ("interior-plateau", "29 metrics"),
                          ("eastern-corn-belt-plains", "21 metrics")):
...
def test_a_documented_gap_is_still_named():
    ...
    assert "16 of 20 functions" in counts and "(4 documented)" in counts
...
def test_the_unassessed_functions_are_named_not_merely_absent():
    """Four Eastern Corn Belt Plains functions have no scoring block. They used to
    be absent from the walk and from the bundle's function list alike, so the only
    trace was one line in the rail."""
    ...
    assert [u["functionName"] for u in un] == [
        "Nutrient cycling", "Water & soil quality", "Population support",
        "Community dynamics"]
...
def test_the_walk_covers_the_whole_framework():
    ...
    for aid in ("northeastern-highlands", "eastern-corn-belt-plains"):
