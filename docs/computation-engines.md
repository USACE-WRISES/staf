...
| Tier | App | Policy |
|---|---|---|
| Screening | EASI | Within 150 ft of an NHDPlus V2 reach, the StreamCat lookup engine supplies watershed metrics in seconds. Elsewhere the STAF site engine calculates the HR reach watershed. Reach-keyed metrics, including the modeled benthic probability and the integrity fallbacks, can use the nearest StreamCat reach downstream; their source details include the routed distance and drainage-area ratio. If the engine cannot compute the watershed, the watershed metrics are unavailable with guidance, never a stand-in. |
