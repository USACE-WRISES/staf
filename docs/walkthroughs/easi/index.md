...
<p>EASI scores any stream in the National Hydrography Dataset. The map draws the high-resolution NHD once and colors each stretch by the engine that answers a click there: dark blue where the StreamCat lookup engine answers the watershed metrics from EPA StreamCat in seconds (a click within 150 ft of an NHDPlus V2 reach), cyan where the STAF site engine calculates the HR reach watershed, mostly small headwaters. After a click the scored StreamCat reach is highlighted on the map. Clicking a cyan stretch makes EASI calculate the HR reach watershed (the drainage area of the high-resolution reach the click snaps to) with the STAF site engine, which usually takes well under a minute (up to about five minutes on a large basin) and shows its progress. Only the three reach-keyed metrics (low flow, substrate, biological integrity) then come from the nearest StreamCat reach downstream, and each of them says so with the reach, the routed distance, and the drainage-area ratio, so the assessment completes with every metric scored. Every value names the engine that produced it. The two engines are defined on the <a href="{{ site.baseurl }}/computation-engines/">Computation Engines</a> page.</p>
