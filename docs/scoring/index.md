...
<details class="tier-how-it-works">
  ...
  <p>Most functions are rated from a single indicator per tier. Where a Screening-tier metric reads more than one indicator, the more limiting one governs the rating. A low level of pressure on one indicator cannot offset a high level on another, because the indicators are not averaged. EASI's Catchment hydrology metric is the clearest example, rating the worse of watershed impervious cover and agricultural cover. The <a href="{{ site.baseurl }}/walkthroughs/easi/">EASI metric reference</a> lists each indicator's boundaries and which one governed.</p>
