...
| SFARI | STAF site engine for the exact watershed and every watershed value at every site (one NHD network on the map, every click snaps to it). No StreamCat substitution: a value is pending while the engine runs and unavailable with the reason when it fails. Direct services answer the rest. The assessor scores; the evidence is labeled. | None | Evidence badges, tooltips, the printed field packet, the appendix |
| DEEP | The exact watershed at every site; auto-pulled values from the STAF site engine for all eight landscape metrics when the bundle's `predictorSource` records engine predictors, and from the StreamCat lookup engine by COMID for StreamCat-built bundles until an engine-fitted assessment replaces them (the train/serve pairing rule). | None, follows the bundle | The source row, the basis badge, the scoring advisory, the exports, the field packet |
| StreamCurves | Predictor source: StreamCat lookup engine (default) or STAF site engine, selected in the region builder or with `--predictor-source`. An engine-sourced build also recomputes the eight scored landscape metrics (impervious, crop, woody and herbaceous wetland, road density, dam density, base-flow index from the USGS grid, road-stream crossings on the NHDPlus HR network) at every retained site and stamps those curves per metric. The reference screen is pinned to `streamcat-legacy`. | Predictor source only | Manifest `inputs.predictor_source` (with `resourced_metrics`), bundle `predictorSource` (bundle-level, and per metric on the re-sourced curves), the science support report |
