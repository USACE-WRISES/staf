...
You are helping a research engineer choose how to distribute desktop versions of existing web-based Shiny applications inside a locked-down U.S. federal (USACE) Windows environment. The briefing below is written to be neutral and should not be read as favoring any option. Using the context, constraints, and tradeoffs provided:
...
- End users run locked-down federal Windows machines, frequently without local administrator rights.
...
5. A distribution route that can realistically be approved for federal deployment.
...
- Maturity: Electron and electron-builder are mainstream and heavily used. shinyelectron specifically is labeled experimental and prototype and is essentially a single-maintainer project, which is a consideration for production federal use.
...
- Code signing is effectively mandatory for federal distribution. The certificate choice (OV versus EV or Azure Trusted Signing) affects how quickly SmartScreen stops warning, and hardware or HSM key storage is now required.
